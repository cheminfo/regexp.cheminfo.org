...
test('a deployment on a host of its own writes its addresses unchanged', async () => {
  const site = await siteMountedAt('https://regexp.cheminfo.org/');
...
test('a deployment mounted under a path writes every address under it', async () => {
  const site = await siteMountedAt('https://www.cheminfo.org/regexp/');

  expect(site.BASE_PATH).toBe('/regexp');
  expect(site.withBase('/')).toBe('/regexp/');
  expect(site.withBase('/exercises')).toBe('/regexp/exercises');
  expect(site.pathWithoutBase('/regexp/exercises')).toBe('/exercises');
  expect(site.pathWithoutBase('/regexp')).toBe('/');
});

test('the same build serves both addresses, because the mount is not built in', async () => {
  const own = await siteMountedAt('https://regexp.cheminfo.org/');
  const shared = await siteMountedAt('https://www.cheminfo.org/regexp/');

  expect(own.withBase('/about')).toBe('/about');
  expect(shared.withBase('/about')).toBe('/regexp/about');
});
...
test('a page of another tool on the shared host is not read as one of ours', async () => {
  ...
  expect(site.pathWithoutBase('/regexpon')).toBe('/regexpon');
});

test('the published address is the default until the build is told another', () => {
  expect(configuredSiteUrl()).toBe('https://regexp.cheminfo.org/');

  vi.stubGlobal('process', {
    env: { SITE_URL: 'https://example.org/regexp/' },
  });

  expect(configuredSiteUrl()).toBe('https://example.org/regexp/');
});
